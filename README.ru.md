...
| Бенчмарк партнёра (`src/benchmark.py`, `reports/model_comparison.json`): LightGBM с L2 против L1 против CatBoost, среднее по фолдам | 0.1717 / **0.1636** / 0.1695 | цель L1 это первый кандидат на следующую итерацию |
| Исследование нейросетей партнёра (`src/deep.py`, `reports/deep_comparison.json`): RNN, LSTM, BiLSTM, малые MLP против LightGBM на одной оценке | сети 0.150–0.159, LightGBM L1 0.146, LightGBM L2 0.164 | остаются деревья; сети не окупаются при таком объёме данных |
| Независимый конвейер третьего участника (`power_forecasting/`): LightGBM на 15 признаках, walk-forward по четырём последним месяцам с as-of чекпоинтами, 42 своих теста | в целом 0.151, D1 0.142, D2 0.159 (своя валидация, напрямую не сравнима) | оставлен как независимое подтверждение уровня ошибки; агентного цикла в нём нет |
...
с модулем ошибки ветра); сдвиг часов SCADA по нашим двум методам UTC+6, по методу третьего
...
1. **Точные запуски NWP** из сырых GRIB-архивов (NOAA GFS `s3://noaa-gfs-bdp-pds`, ECMWF Open
   ...
   `strict`.
2. **Цель L1 и признаки качества провайдеров по скользящему окну** из экспериментов партнёра и
...
модель: `artifacts/pooled_model.joblib`. Эксперименты партнёра: `reports/model_comparison.json`,
`reports/deep_comparison.json`; независимый конвейер: `power_forecasting/` со своим README.
...
точность здесь не считается; `power_forecasting/predictions/final_forecast.csv` это независимая
оценка третьего участника, а не файл сдачи.
...
src/
  ...
  benchmark.py, deep.py   эксперименты партнёра (опционально, requirements-experiments.txt)
tests/                 офлайн-тесты честности (pytest)
power_forecasting/     независимый конвейер третьего участника, свой README и тесты
...
reports/               картинки и результаты экспериментов партнёра
